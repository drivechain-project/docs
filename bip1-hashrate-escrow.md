...
One way to think of this feature is as a 2-of-3 escrow, where the 3rd party (who will arbitrate any disputes) is the set of Bitcoin Miners. In practice these escrows are likely to be "asymmetric sidechains" of Bitcoin (such as [Rootstock](http://www.rsk.co/)) or "virtual chains" within Bitcoin (such as [proposed by Blockstack](https://github.com/blockstack/virtualchain) in mid-2016).

"Hashrate Escrow" is a clearer way of expressing the (completely-equivalent) concept of "locked to an SPV Proof" which is itself a restatement of the phrase "within a sidechain" as described in [a famous paper](https://blockstream.com/sidechains.pdf) written partially by some Blockstream co-founders.


### Components

Hashrate Escrows are built of several components. These components are of one of two types: new databases, or new message-interpretations.
...
2 | Active\* | 2 | uInt | This counter starts at zero when a new entry is added, and increments as a new entry is supported by "M2". The entry either reaches the max value of 3000 (at which point the counter freezes), or else the entry is removed. This enforces the requirement "a soft fork is required to add a new escrow".
...
Currently, we *literally sign* a txn with a globally and publicly known private key. But this is for convenience purposes -- the signature that is produced is not doing anything. Instead we may use OP_TRUE, but this might interfere with how we detect the sidechain's balance. I'm not sure what the best way is. Someone needs to investigate how to do this -- removing OP_CheckSig, etc. This is a TODO for sure.
...
1. A hash commitment to D2 must always exist.
...
First, for nodes which validate all sidechains (assuming these escrows are sidechains), this simplifies to 2^n -- these nodes only have to choose between the single honest choice or an abstention. Second, even for nodes that don't validate any sidechains, the number of candidates might be reduced to 3^n, by making a simplifying assumption: whichever withdrawal was most recently added/upvoted, is likely to be the one which is upvoted next.
...
If the message is very very large, then nodes may see not want to broadcast it. This opens up an "exhaustion attack"<sup>2</sup>. Fortunately even for 200 sidechains and 1,000 withdrawal-attempts each (absurdly unrealistic), the long form M4 is just over 25 KB. (Meanwhile, to initiate this attack, the attacker must give up about 7722 KB [ 200\*990\*(1+1+4+33)] in regular blockspace.) Instead of creating bad WT^s and refusing to explain them with 25 KB M4s, an attacking miner could just include a regular 200 byte txn and refuse to ever broadcast it.
...
From there, we merely clarify two additional points:
